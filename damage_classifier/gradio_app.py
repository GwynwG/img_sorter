    ...
    for summary in summaries:
        rows.append(
            ...
            <tr>
                ...
                <td>{_format_path(summary.best_path)}</td>
                <td>{_format_path(summary.results_csv)}</td>
            </tr>
            """
        )
    return f"""
...
def _artifact_gallery_items(summaries: list[ModelArtifactSummary] | None = None) -> list[str]:
    summaries = summaries or summarize_all_artifacts()
    items: list[str] = []
    for summary in summaries:
        for path in (summary.results_png, summary.confusion_matrix, summary.confusion_matrix_normalized):
            if path is not None:
    ...
    with gr.Blocks(title="模拟损伤光斑图分级识别") as demo:
        with gr.Column(elem_id="app-shell"):
            ...
            def refresh_analysis():
                ...
                return _build_artifact_summary_html(summaries), _artifact_gallery_items(summaries)
            ...
            with gr.Tabs():
                ...
                with gr.Tab("模型分析"):
                    ...
                    analysis_refresh.click(
                        fn=refresh_analysis,
                        inputs=[],
                        outputs=[analysis_html, analysis_gallery],
                    )
